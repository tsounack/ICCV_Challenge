...
from torch.utils.data.sampler import BatchSampler, SequentialSampler, RandomSampler, WeightedRandomSampler
...
def create_model(config, dl, logger, from_training=True, state_dict=None):
    ...
    if state_dict is not None and "model" in state_dict:
        ...
        if 'loss_func.pos_weight' in params:
            params.pop('loss_func.pos_weight')
...
def create_data_loader(config, split, logger, called_by_validator=False, called_by_ensemblor=False):
    ...
    if split == 'train' and not called_by_validator:
        # num_train_images = int(len(dataset) / 20)
        ...
        class_weights = 1 / label_freqs
        stacked_labels = torch.stack(dataset.label.labels, axis=0)
        label_weights = torch.max(stacked_labels * torch.from_numpy(class_weights), axis=1)

        weighted_sampler = WeightedRandomSampler(label_weights.values, num_train_images, replacement=True)
